  ...
  function renderBuilder() {
    ...
    el.innerHTML = `
      ...
      <div class="card">
        ...
        <!-- XP Mult -->
        <div class="builder-field"><label>${t("builder_xp_mult")}: <span class="range-val" id="bXV">×1.0</span></label>
          <input type="range" id="bX" min="10" max="30" value="10" step="1"></div>
    ...
    syncRange("#bX","#bXV", v=>`×${(parseInt(v)/10).toFixed(1)}`);
    ...
    el.querySelectorAll("[data-diff]").forEach(b=>b.addEventListener("click",()=>{
      ...
      if (p) {
        ...
        el.querySelector("#bX").value=Math.round(p.xm*10); el.querySelector("#bXV").textContent=`×${p.xm.toFixed(1)}`;
      }
    }));
    ...
    el.querySelector("#bSave").addEventListener("click",()=>{
      ...
      const mode = {
        ...
        xpMult:    parseFloat((parseInt(el.querySelector("#bX").value)/10).toFixed(1))
      };
